...
把「排班、每日/月度任务、定时 Webhook 推送、整月可视矩阵」收进一个自托管工作台：管理者排班派活，成员到点收到任务汇总与当班提醒；每日/月度任务按周期自动重置、逾期自动判定，昨天没做完的不会凭空消失。再加一块「**工作台**」（迷你知识库 / 工作日志 / 交接接力，v0.6.0+）：每天做了什么随手记下来、没做完的标明后交给下一个人接着做，知识方法可带附件沉淀共享，数据支持一键导出打包、可多环境迁移。
...
### 工作台（v0.6.0+）
面向「每天做了什么、还没做完的接着做」的轻量协作本，三大子模块，每条数据可设 **同部门共享 / 仅自己** 两种可见范围（部门隔离，超管可读全部）：

- **迷你知识库**：把方法 / 流程 / 话术 / SOP 沉淀成条目，支持全文搜索、分类标签、「只看我写的」筛选；列表卡片 + 点击详情
  - **附件**（v0.7.0+）：任意文件类型、单文件 ≤ 100MB；图片缩略图并可点击预览，其余类型图标 + 点击下载；上传 / 删除权限归条目创建者、上传者与超管，可见者均可下载
  - **导出知识**（v0.7.0+）：把当前筛选可见的知识导出为带 BOM 的 `.txt`（按分类分组，Excel / 记事本打开不乱码）
- **工作日志**：按日期记录「✅今天做了什么 / ⏳还没做完的」，一天可多篇；可写主题，支持「我写的 / 部门共享日志」切换——没做完的写清楚，方便组内互见或次日接着做
- **交接接力**：把「做到哪、接下来做什么」成点对点交接给下一个接手人；接收人收到**站内通知**，可流转 待接手 → 处理中 → 已完成，处理中可更新进度备注，完成时填完成说明；「我收到的 / 我发出的」两个视角，超管可统筹全部
- **一键打包下载 zip**（v0.7.0+）：把当前用户可见的**知识 + 日志 + 交接（README.txt）+ 全部附件**汇总打包，用于备份或在多套环境间迁移数据
...
| 版本 | 变更类型 | 内容 |
|---|---|---|
| **v0.7.0** | 增强 | 迷你知识库支持**任意类型附件**（≤100MB，图片预览/其余下载，按扩展名识别 MIME）；知识可导出为带 BOM 的 `.txt`；新增**一键打包下载 zip**（知识+日志+交接 README + 全部附件，用于跨环境迁移）；导出/打包严格按可见范围过滤，修复打包日志越权泄漏；附件落盘随数据目录便于整目录迁移 |
| **v0.6.0** | 增强 | 新增**「工作台」模块**：迷你知识库（沉淀方法/流程，卡片+搜索+分类）、工作日志（一天可多篇，「✅做了什么/⏳没做完的」）、交接接力（点对点转给下一个人，含站内通知与 待接手→处理中→已完成 状态流转）；每条数据可设 **同部门共享 / 仅自己**，部门隔离 |
| **v0.5.0** | 修复 | 6 项：版权信息压缩展示、去除测试用 admin/客服部/运维部 默认部门、林晓/陈默改为真实可管理用户、登录记录真实客户端 IP、备份导入 UI 入口优化、任务批量删除/批量操作、整月矩阵视图美化 |
| **v0.4.0** | 增强 | 产品化安装体验：新增 `install.sh` 一键脚本（自动装 Docker、生成密钥、拉镜像、启动），新库 admin 首登强制改密 |
| **v0.3.0** | 增强 | 密码安全加固：至少 8 位且含字母数字，全员强制改密 |
...
> 语义化版本说明：v0.0.1–v0.0.5 → v0.1.0 → v0.2.0 → v0.2.1 → v0.2.2 → v0.3.0 → v0.4.0 → v0.5.0 → v0.6.0 → v0.7.0，release.sh 从最新 tag 自动 bump。
