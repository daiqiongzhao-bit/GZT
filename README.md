...
- **推送名单与@分离**（v0.0.1 起）：用户可标记「已加入企业微信通知群」，推送只 @ 已入群成员，当班名单只显示未入群人员，不再重复列出
...
| 版本 | 变更类型 | 内容 |
|---|---|---|
| v0.0.1 | 初始 | 企业排班任务工作台全新起点：班表管理、任务（每日/月度/单次+周期自动重置）、Webhook 定时推送与@、RBAC 三级权限、账号=工号、手机号归一化、导入导出、PWA 三端 + 浏览器插件、推送名单与@分离（in_group 标记） |
