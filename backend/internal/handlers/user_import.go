...
func getUserTemplateXLSX() *excelize.File {
	...
	f.SetCellValue(sheet, "A11", "5. 再次导入（工号已存在）只更新资料、不会改动原密码；如需重置密码请在该员工档案点「重置密码」。新员工导入时「初始密码」生效（留空默认 Cdf123456）。")
	f.SetCellValue(sheet, "A12", "6. 跨服务器迁移：目标服务器为空系统时导入=全部按新员工创建、初始密码列生效；目标服务器已有同名账号则按第 5 条只更新资料。完整数据迁移请用「备份下载→新服务器还原」或「工作台全库打包」。")
	return f
}
...
func DownloadUserTemplateXLSX(c *gin.Context) {
	...
	c.Header("Content-Disposition", fmt.Sprintf("attachment; filename=\"%s\"; filename*=UTF-8''%s", safeDispName("人员导入模板.xlsx"), url.QueryEscape("人员导入模板.xlsx")))
	_ = f.Write(c.Writer)
}
